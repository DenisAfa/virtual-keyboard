const Keyboard = {
        ...
        const keyLayout = [
          ...
          keyLayout.forEach(key => {
            ...
            if (window.matchMedia("(min-width: 800px)").matches) {
                insertLineBreak = ["backspace", "]", "enter", "en"];
            } else {
                insertLineBreak = ["backspace", "]", "/"];
            }
            ...
            switch (key[this.properties.language]) {
                ...
                case "space":
                    ...
                    keyElement.addEventListener("click", () => {
                        let area = document.querySelector(".use-keyboard-input")
                        this._specialPlaySound(key);
                        this.properties.value += area.setRangeText(" ", area.selectionStart, area.selectionEnd, "end");
                        area.focus();
                        this._triggerEvent("oninput");
    ...
    _playSound() {
        if (this.properties.sound) {
            if (this.properties.language === "en") {
                const soundEng = document.querySelector(".key__audio_en");
                soundEng.play()
            } else if (this.properties.language === "ru") {
                const soundRu = document.querySelector(".key__audio_ru");
                soundRu.play()
            }
        };    
    },

    _specialPlaySound(key) {
        if (this.properties.sound) {
            if (key[this.properties.language] === "space") {
                const soundSpace = document.querySelector(".key__audio_space");
                soundSpace.play()
            } else if (key[this.properties.language] === "shift") {
                const soundShift = document.querySelector(".key__audio_shift");
                soundShift.play()
            } else if (key[this.properties.language] === "enter") {
                const soundEnter = document.querySelector(".key__audio_enter");
                soundEnter.play()
            } else if (key[this.properties.language] === "caps") {
                const soundCaps = document.querySelector(".key__audio_caps");
                soundCaps.play()
            } else if (key[this.properties.language] === "backspace") {
                const soundBackspace = document.querySelector(".key__audio_backspace");
                soundBackspace.play()
            }
        }; 
    },
    ...
    _speech() {
        ...
        recognition.lang = this.properties.language === 'en' ? 'en-US' : 'ru-RUS'
